  ...
  const menuTemplate: MenuItemConstructorOptions[] = [
    {
      label: "Apply",
      async click() {
        await provider.stashApply(currentRepo(), Number.parseInt(data.index, 10));
      },
    },
    {
      label: "Pop",
      async click() {
        await provider.stashPop(currentRepo(), Number.parseInt(data.index, 10));
      },
    },
    {
      label: "Drop",
      async click() {
        await provider.stashDrop(currentRepo(), Number.parseInt(data.index, 10));
      },
    },
  ];
  Menu.buildFromTemplate(menuTemplate).popup();
