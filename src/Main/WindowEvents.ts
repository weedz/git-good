...
export function requestClientData<T extends RendererRequestEvents>(
  event: T,
  args: RendererRequestArgs[T],
) {
  ...
  return new Promise<null | RendererRequestData[T]>((resolve) => {
    ...
    callbackHandlers[id] = resolve as unknown as (
      args: RendererRequestData[RendererRequestEvents],
    ) => void;
  });
}
