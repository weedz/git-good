...
export abstract class StoreComponent<P = unknown, S = unknown> extends Component<P, S> {
  ...
  listen<T extends StoreKeys>(
    key: T,
    cb: PartialStoreListener<StoreType, T> = () => this.forceUpdate(),
  ) {
...
export function notify(
  notificationData: NotificationInit & { body?: null | string | AnyComponent | JSX.Element },
  _?: unknown,
): Notification {
...
const deleteNotification: { [K in NotificationPosition]: (id: number) => void } = {
  [NotificationPosition.DEFAULT]: (id) =>
    Store.notifications[NotificationPosition.DEFAULT].delete(id) &&
    store.triggerStoreUpdate("notifications"),
};
