      ...
      <div id="working-area" class="pane">
        <div id="unstaged-changes">
          <h4>
            ...
            <button disabled={!this.state.unstaged.length} onClick={stageAllChanges}>
              Stage all
            </button>
            <button disabled={!this.state.unstaged.length} onClick={discardAllChanges}>
              Discard all
            </button>
          </h4>
          <ChangedFiles
            ...
            type="unstaged"
            actions={[
              { label: "Stage", click: stageFile },
              { label: "Discard", click: discard },
            ]}
          />
        </div>
        <div id="staged-changes">
          <h4>
            ...
            <button disabled={!this.state.staged.length} onClick={unstageAllChanges}>
              Unstage all
            </button>
          </h4>
          <ChangedFiles
            patches={this.state.staged}
            workDir
            type="staged"
            actions={[{ label: "Unstage", click: unstageFile }]}
          />
        </div>
        <div>
