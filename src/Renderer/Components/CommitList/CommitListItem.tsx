    ...
    <ul class="commit-graph">
      {props.graphCommit.descendants.map((child) => {
        ...
        return (
          <li key={child.sha}>
            <Link
              selectTarget={() => GlobalLinks[LinkTypes.COMMITS][child.sha]}
              style={{ color: HeadColors[commit.colorId].color }}
            >
...
function GraphContainer(
  ...
  return (
    <div class="graph-container">
      ...
      />
      {props.graphCommit.descendants.length > 0 && (
        <CommitGraph graph={props.graph} graphCommit={props.graphCommit} />
      )}
    </div>
  );
}

function CommitReferences(props: { head: BranchObj[]; graphCommit: CommonProps["graphCommit"] }) {
  return (
...
function formatCommitTime(date: Date) {
  return (
    `${date.getFullYear()}-${(date.getMonth() + 1).toString(10).padStart(2, "0")}-${date.getDate().toString(10).padStart(2, "0")}` +
    " @ " +
...
export default function CommitListItem(props: Props) {
  ...
  return (
    <li class="short" style={props.style}>
      ...
      <GraphContainer
        isMerge={props.commit.parents.length > 1}
