      ...
      <li onContextMenu={contextMenu} key={patch.actualFile.path} data-path={patch.actualFile.path}>
        ...
        <div class="action-group">
          {checkActions(patch, actions).map((action) => (
            <button
              key={patch.actualFile.path}
              data-path={patch.actualFile.path}
...
function renderTree(
  ...
  const sortedChildren = Array.from(tree.children.entries()).sort(([_, a], [_b, b]) =>
    a.children.size && !b.children.size ? -1 : 0,
  );
...
export default class ChangedFiles extends Component<Props, State> {
  ...
  render() {
    ...
    const patches = fileFilter
      ? this.props.patches.filter((patch) =>
          patch.actualFile.path.toLocaleLowerCase().includes(fileFilter),
        )
      : this.props.patches;
    ...
    const files =
      this.state.renderType === RenderType.PATH
        ? renderPaths(
            patches.slice(0, 1000),
            this.props.actions || [],
            this.fileContextMenu,
            this.openFile,
          )
        : renderTreeFromPatches(
            patches.slice(0, 1000),
            this.props.actions || [],
            this.fileContextMenu,
            this.openFile,
    ...
    return (
      <div class="changed-files inset">
        <div class="flex-row btn-group" style="margin: auto">
          ...
          {this.state.renderType === RenderType.TREE && (
            <span
              ...
              onClick={(e) => {
                const fileContainer = e.currentTarget
                  .closest(".changed-files")
                  ?.querySelector(".diff-view.block-list");
                if (fileContainer) {
        ...
        <Links.Provider value={LinkTypes.FILES}>
          <ul class="diff-view block-list">{files}</ul>
        </Links.Provider>
        <input
          type="text"
          onKeyUp={this.filterFiles}
