    ...
    return (
        <div class="commit-message">
            <div class="pane commit-header">
                {signature}
                <span class="commit-sha pointer" title="Copy sha" onClick={() => navigator.clipboard.writeText(props.commit.sha)}>
            ...
            <p>
                ...
                <ul class="parent-list">
                    {props.commit.parents.map(parent => (
                        <li key={parent.sha}>
                            <Link linkType={LinkTypes.COMMITS} selectTarget={() => GlobalLinks[LinkTypes.COMMITS][parent.sha]}>
            ...
            {props.commit.date !== props.commit.authorDate && (
                <p class="date">Authored: {authorDate.toISOString().substring(0, 19)}Z ({formatTimeAgo(authorDate)})</p>
            )}
            ...
            {props.commit.committer.email !== props.commit.author.email && (
                <p class="author">commiter: {props.commit.committer.name} &lt;{props.commit.committer.email}&gt;</p>
            )}
        </div>
    );
}
