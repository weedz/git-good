...
export function RenderBranchTree(props: {
  ...
  const sortedChildren = Array.from(props.branches.children.entries()).sort(([_, a], [_b, b]) =>
    ...
    if (child.item) {
      ...
      items.push(
        <li class="sub-tree" key={item}>
          <a style={{ textIndent: `${props.indent}em` }} href="#" onClick={toggleTreeItem}>
            {item}
          </a>
          <RenderBranchTree
            branches={child}
  ...
  return <ul class="tree-list block-list">{items}</ul>;
}
    ...
    items.push(
      <li class="sub-tree" key={item}>
        <a
          ...
          onContextMenu={props.remoteContextMenu}
          data-remote={item}
        >
          {item}
        </a>
