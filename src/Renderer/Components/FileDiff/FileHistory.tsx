...
export default class CommitContainer extends Component<Props, State> {
  ...
  render() {
    const fileHistory = this.state.filter
      ? this.props.fileHistory.filter((item) => filterCommit(this.state.filter, item))
      : this.props.fileHistory;
    return (
      <div id="file-history-commits" class="pane">
        ...
        <input
          ...
          title="Search/Filter"
          onInput={(e) => this.updateFilter(e.currentTarget.value)}
        />
              ...
              renderItems={(commits, start) => {
                ...
                return commits.map((commit, idx) => {
                  ...
                  if (commit.status === DiffDelta.ADDED) {
                    ...
                    action = (
                      <Link
                        selectAction={() => this.props.openFileHistory(commit.path)}
                        title={commit.path}
                      >
                  ...
                  const item = (
                    <li
                      style={{
                        position: "absolute",
                        top: `${(start + idx) * ITEM_HEIGHT + extraHeightOffset}px`,
                        height,
                      }}
                      key={commit.sha}
                    >
                      <Link
                        ...
                        selectAction={async (_arg) => {
                          const filePatch = await ipcGetData(IpcAction.FILE_DIFF_AT, {
                            file: commit.path,
                            sha: commit.sha,
                          });
                          if (filePatch) {
