...
export default abstract class ScrollListView<T, P = unknown> extends Component<
  Props<T> & P,
  State
> {
  sync = false;
  ...
  checkScrollPosition = () => {
    if (this.containerRef.current) {
      ...
      const startLine = Math.max(
        0,
        Math.floor(this.containerRef.current.scrollTop / this.props.itemHeight) -
          EXTRA_ITEMS_TO_RENDE,
      );
